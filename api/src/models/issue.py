...
from src.models.base import Base, UUIDPKMixin, TimestampMixin, SoftDeleteMixin
...
class Issue(Base, UUIDPKMixin, TimestampMixin, SoftDeleteMixin):
    ...
    ai_confidence: Mapped[float | None] = mapped_column(Float)  # 0-1; low → human review
    ...
    # Cached session context (legacy — primary memory in event log + IssueMemory projection)
