...
**Date:** 2026-05-18
**Status:** Plan v1.0 — design locked. Ready for Phase 0.
...
- Arbitrary plugin SDK (only MCP + provider adapter)
...
## 8. Cutover Plan — 9 Phases, ~13.5 weeks

### Phase 0 — Greenfield setup (1 week)
...
### Phase 6 — Knowledge memory + Skills + Memory tracking (1.5 weeks)
...
### Phase 7 — ChatSession + Notifications + Replay UI (2 weeks)
...
**Total: ~13.5 weeks** single full-time. Adjust for team size.
...
- Plugin SDK beyond MCP + provider adapter
...
1. **Create new repo `bumblebee`** (Phase 0 start)
2. **Scaffold:** monorepo structure (api/, cli/, web/, workflows/), LangGraph + FastAPI base, PG schema for events + projections, CI workflows
3. **Document Phase 0 setup** in `bumblebee/docs/getting-started.md`
4. **Begin Phase 1:** single-agent E2E
...
**Status:** Plan v1.0 LOCKED. All design decisions resolved. Ready for Phase 0 execution.
