"""Test: PG SKIP LOCKED queue â€” enqueue + claim + complete + fail."""
import uuid
import pytest

from bumblebee.services.dispatch.task_queue import enqueue, claim_next, complete, fail
...
async def test_fail_retries_then_dead_letter(clean_db):
    ...
    # First claim â†’ fail (attempts=1, re-queued)
    c1 = await claim_next(db, claimed_by="w1")
    await db.commit()
    await fail(db, c1["id"], "transient")
    await db.commit()

    # Second claim â†’ fail (attempts=2 = max, â†’ dead_letter)
