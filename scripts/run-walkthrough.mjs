...
(async () => {
  ...
  // ============ STEP 8: Create new issue (UI screenshot + API for reliability) ============
  {
    const s = step("create-issue-via-web");
    await page.goto(`${WEB}/issues`, { waitUntil: "networkidle" });
    await page.waitForTimeout(800);
    // Try web form first (best-effort, capture screenshot for docs)
    const input = page.locator('input[placeholder*="title"]').first();
    await input.click();
    await input.pressSequentially("E2E Walkthrough Real Claude classify", { delay: 30 });
    await page.waitForTimeout(500);
    await shoot(page, s, "form-filled");
    // Reliable creation via API (browser-based form fill flaky in headless)
    const created = await call("POST", "/api/projects/bb/issues", {
      title: "E2E Walkthrough Real Claude classify",
      description: "Created via admin walkthrough script — exercises full multi-node workflow",
      type: "task",
      priority: "high",
    }, { Authorization: `Bearer ${bearer}` });
    issueId = created.body.id;
    s.actions.push({
      type: "api", method: "POST", path: "/api/projects/bb/issues",
      status: created.status,
      result: `BB-${created.body.number} created via API (X-BB-Bearer)`,
    });
    console.log(`  ✓ created BB-${created.body.number} via API`);
    // Reload to show new item in list
    await page.reload({ waitUntil: "networkidle" });
    await page.waitForTimeout(1000);
    await shoot(page, s, "list-with-new-issue");
    transcript.push(s);
  }
  ...
  // ============ STEP 10: Trigger workflow ============
  {
    ...
    // Screenshot pre-trigger state
    await shoot(page, s, "before-trigger");
    // Try UI button (best-effort); fall back to API
    const triggerBtn = page.locator("button").filter({ hasText: /Trigger/i }).first();
    const visible = await triggerBtn.isVisible().catch(() => false);
    if (visible) {
      await triggerBtn.click();
      console.log(`  ✓ clicked Trigger Workflow button`);
    } else {
      console.log(`  → triggering via API (button not found)`);
      const trig = await call("POST", "/api/workflow-runs/trigger",
        { issue_id: issueId },
        { Authorization: `Bearer ${bearer}` });
      s.actions.push({ type: "api", method: "POST", path: "/api/workflow-runs/trigger", status: trig.status, response: trig.body });
    }
    // Wait for completion (poll up to 90s for real claude)
    let done = false;
    for (let i = 0; i < 90; i++) {
      const events = await call("GET", `/api/events?issue_id=${issueId}&limit=30`);
      const types = events.body.map((e) => e.type);
      if (types.includes("workflow_completed") || types.includes("workflow_failed")) { done = true; break; }
      await new Promise((r) => setTimeout(r, 1000));
    }
    await page.reload({ waitUntil: "networkidle" });
    await page.waitForTimeout(1500);
    await shoot(page, s, "after-completion");
